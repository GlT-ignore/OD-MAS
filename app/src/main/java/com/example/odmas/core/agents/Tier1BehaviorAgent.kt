...
/**
 * Tier-1 Behavioral Agent implementing the user's new spec:
 * - Calibration is coverage-based, not time-based:
 *   Motion: hold naturally for ~10s (we approximate via N motion windows)
 *   Touch: ~30 taps (we approximate via N touch windows received)
 *   Typing: prompt paragraph; we gate on key events count and space/bigram observations
 *
 * - Per-modality baseline models:
 *   MotionModel: stationary mean/std (accel magnitude minus gravity), tremorLevel baseline
 *   TouchModel: dwell, interval, pressure, size statistics
 *   TypingModel: dwell and flight distributions (mean/std and quartiles), space-bar rhythm, wpm, bigram timings (basic)
 *
 * - Real-time 3s window scoring: compute per-modality probability in [0,1], where larger = more anomalous.
 *
 * Notes and pragmatic adaptations:
 * - This agent receives per-window 10-D feature vectors (not raw 50Hz streams). We approximate tremor via feature[7]
 *   coming from MotionSensorCollector, and use window-level statistics to calibrate.
 * - For Typing, fine-grained dwell/flight and space detection are fed via SecurityManager.onTypingEvent(...).
 */
class Tier1BehaviorAgent(private val context: Context) {
    ...
    // ---------- Calibration Buffers ----------
    private val motionAccelNoG = mutableListOf<Double>()  // accel magnitude minus gravity
    private val motionTremorLevels = mutableListOf<Double>() // feature[7] tremor level
    ...
    // ---------- Calibration thresholds (pragmatic, window-level approximations) ----------
    private val MOTION_WINDOWS_NEEDED = 4           // ~12s if ~3s per window
    ...
    // Back-compat readiness for UI/logs: any modality modeled yet?
    fun isAnyModalityReady(): Boolean {
        return (motionModel != null) || (touchModel != null) || (typingModel != null)
    }

    // Return aggregate calibration progress percent (0..100)
    fun getCalibrationProgressPercent(): Int {
        ...
        val avg = (pm + pt + pk) / 3.0
        return avg.toInt()
    }

    // Expose per‑modality counters for UI/notifications
    fun getMotionProgress(): Pair<Int, Int> = motionWindows to MOTION_WINDOWS_NEEDED
    ...
    // Reset all calibration buffers and models
    fun resetBaseline() {
    ...
    // Back-compat API used by SecurityManager/seed demo
    fun addBaselineSample(features: DoubleArray, modality: Modality) {
        // Reuse window ingestion
        submitCalibrationSample(features, modality)
        // If typing, synthesize per-key dwells/flights from window features to advance key counter
    ...
    // Finalize models if thresholds are satisfied
    ...
    // Lifecycle placeholder for parity with older agents
    fun close() {
        // No resources to release in this pure-Kotlin implementation
    }

    // Submit 10-D window features during calibration phase
    fun submitCalibrationSample(features: DoubleArray, modality: Modality) {
        if (features.size != 10) return
        when (modality) {
            Modality.MOTION -> {
                motionWindows++
                // Motion features mapping (per MotionSensorCollector):
                // 0: accelMagnitude, 7: tremorLevel
                ...
                if (motionModel == null && motionWindows >= MOTION_WINDOWS_NEEDED) {
                    motionModel = buildMotionModel()
                }
            }
            Modality.TOUCH -> {
                touchWindows++
                // Touch features mapping (per TouchSensorCollector/ViewModel):
                // 4: dwellSec, 2: pressureMean, 3: sizeMean
                ...
                touchSizes.add(size)
                // interval between touch windows (rough proxy)
                val last = lastTouchWindowTimeMs
                if (last != null) {
                    touchIntervals.add((now - last).toDouble() / 1000.0)
                }
                lastTouchWindowTimeMs = now
                if (touchModel == null && touchWindows >= TOUCH_TAPS_NEEDED) {
                    touchModel = buildTouchModel()
                }
            }
            Modality.TYPING -> {
                // Typing is fed via submitTypingTiming(...) per key; calibration finalization happens there.
                // Here we keep window-level alignment only if needed in future.
            }
            else -> {}
        }
    }

    // Submit per-key typing timing during calibration/monitoring
    fun submitTypingTiming(dwellMs: Long, flightMs: Long, isSpace: Boolean) {
        ...
        if (typingModel == null && typingKeys >= TYPING_KEYS_NEEDED) {
            typingModel = buildTypingModel()
        }
    }

    // Compute Tier‑1 probability [0,1] for a 10-D window (per modality)
    ...
    private fun buildTouchModel(): TouchModel {
        val dwellMS = touchDwells.map { it * 1000.0 } // convert to ms for median calculation if needed
    ...
    private fun buildTypingModel(): TypingModel {
        ...
        for (i in 1 until spaceTimestampsMs.size) {
            spaceIntervalsMs.add(spaceTimestampsMs[i] - spaceTimestampsMs[i - 1])
        }
    ...
    private fun scoreMotion(features: DoubleArray, model: MotionModel): Double {
        ...
        var deviations = mutableListOf<Double>()
        deviations.add(accelDev)

        // If near stationary band, tremor pattern (approximate via level deviation) matters more
        if (accelNoG < model.stationaryMean + model.stationaryStd) {
            ...
            if (tremorDev > 0.5) deviations.add(2.0) // push anomaly if tremor "pattern" differs noticeably
        }

        val avg = deviations.average()
        // map to probability via sigmoid; 2 std dev from baseline -> ~0.5
    ...
    private fun scoreTouch(features: DoubleArray, model: TouchModel): Double {
        ...
        // Emphasize dwell (varies most even with a11y), de-emphasize pressure/size
        val pDwell = sigmoid((zDwell - 1.0) * 1.2)
        val pPS = 0.5 * sigmoid((zPressure - 1.5) * 0.8) + 0.5 * sigmoid((zSize - 1.5) * 0.8)

        // Rhythm check only if intervalStd is meaningful and rhythmVar sane
        ...
        val pRhythm = if (hasValidRhythm) {
            // Compare in log-space to be scale-robust
            val rDev = abs(ln(rhythmVar + 1e-9) - ln(intervalStd2 + 1e-9))
            // centered so small dev -> low anomaly
            sigmoid((rDev - 0.8) * 1.0)
        } else null

        val score = if (pRhythm != null) {
            // Strong dwell focus, small contributions from pressure/size and rhythm
            (0.80 * pDwell + 0.15 * pPS + 0.05 * pRhythm)
        } else {
            (0.85 * pDwell + 0.15 * pPS)
        }
    ...
    private fun scoreTyping(features: DoubleArray, model: TypingModel): Double {
        // We approximate dwellSec = features[4], flightSec = derived mean stored in features[1]? Touch vector semantics differ.
        // For typing window-level features are not guaranteed; rely more on incremental typing events; gracefully degrade:
        ...
        // Rhythm consistency (log-space variance comparison)
        ...
        // WPM check via rough estimation:
        val currentWpm = model.wpm // no good per-window estimator from features; use baseline comparison neutral
    ...
    private fun median(x: List<Long>): Long {
        if (x.isEmpty()) return 0L
        val a = x.sorted()
        val n = a.size
