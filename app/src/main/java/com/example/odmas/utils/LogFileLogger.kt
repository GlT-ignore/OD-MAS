package com.example.odmas.utils

import android.content.Context
import android.util.Log
import java.io.File
import java.text.SimpleDateFormat
import java.util.Date
import java.util.Locale

/**
 * Lightweight file logger.
 *
 * - Writes to internal storage at filesDir/odmas.log
 * - Rotates at ~512 KB to odmas.log.1 (overwrite)
 * - Safe to call from any thread
 */
object LogFileLogger {
    @Volatile
    private var initialized = false
    private lateinit var appContext: Context
    private val dateFormat = SimpleDateFormat("yyyy-MM-dd HH:mm:ss.SSS", Locale.US)
    private const val LOG_FILE_NAME = "odmas.log"
    private const val LOG_FILE_ROTATE = "odmas.log.1"
    private const val MAX_BYTES: Long = 512 * 1024 // 512 KB

    fun init(context: Context) {
        if (!initialized) {
            synchronized(this) {
                if (!initialized) {
                    appContext = context.applicationContext
                    initialized = true
                    log("LogFileLogger", "Initialized file logger")
                }
            }
        }
    }

    fun log(tag: String, message: String, throwable: Throwable? = null) {
        // Always print to Logcat too.
        if (throwable != null) {
            Log.e(tag, message, throwable)
        } else {
            Log.d(tag, message)
        }

        if (!initialized) return

        try {
            val dir: File = appContext.filesDir
            if (!dir.exists()) dir.mkdirs()

            val logFile = File(dir, LOG_FILE_NAME)
            // Rotate if needed
            if (logFile.exists() && logFile.length() >= MAX_BYTES) {
                val rotated = File(dir, LOG_FILE_ROTATE)
                // Best-effort rotation (overwrite old rotated file)
                runCatching {
                    if (rotated.exists()) rotated.delete()
                    logFile.copyTo(rotated, overwrite = true)
                    logFile.writeText("") // truncate
                }
            }

            val ts = dateFormat.format(Date())
            val line = StringBuilder()
                .append(ts)
                .append(" ")
                .append(tag)
                .append(": ")
                .append(message)
                .append('\n')

            if (throwable != null) {
                line.append(throwable.stackTraceToString())
                    .append('\n')
            }

            logFile.appendText(line.toString())
        } catch (_: Throwable) {
            // Swallow to avoid impacting runtime
        }
    }
}